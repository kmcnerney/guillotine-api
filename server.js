...
async function login() {
  try {
    ...
    if (pageSource.includes("Stay&nbsp;verified")) {
      ...
      const stayVerifiedButton = await driver.findElement(
        By.className("puree-button-primary")
      );
...
async function getLiveProjections() {
  ...
  try {
    ...
    if (pageSource.includes("Final results")) {
      ...
      await driver.wait(
        until.elementIsVisible(driver.findElement(By.className("Js-next"))),
        5000
      );
      await driver.wait(
        until.elementIsEnabled(driver.findElement(By.className("Js-next"))),
        5000
      );
    ...
    const leagueTableBody = await leagueTable[0].findElements(
      By.tagName("tbody")
    );
    ...
    for (const team of teams) {
      ...
      const teamCell = await cells[TABLE_CELL_INDICES.TEAM].findElements(
        By.tagName("a")
      );
      const placeCell = await cells[TABLE_CELL_INDICES.TEAM].findElements(
        By.className("Mstart-med")
      );
      scores.push({
        teamName: await teamCell[0].getAttribute("innerHTML"),
        projectedPts: parseFloat(
          await cells[TABLE_CELL_INDICES.PROJECTION].getAttribute("innerHTML")
        ),
        currentPts: parseFloat(
          await cells[TABLE_CELL_INDICES.CURRENT].getAttribute("innerHTML")
        ),
...
app.get("/live-projections", async (req, res) => {
  if (lock) {
    console.log(
      "another user is already checking scores, just give the latest"
    );
